...
public class UnityChanDance : MonoBehaviour
{
        ...
        if (Frame.Raycast(touch.position.x, touch.position.y, raycastFilter, out TrackableHit hit))
        {
            ...
            else
            {
                ...
                else if (hit.Trackable is DetectedPlane)
                {
                    ...
                    {
                        Create(ceilingIndex, ceilingProduct, hit);
                    }
                    else if (detectedPlane.PlaneType == DetectedPlaneType.HorizontalUpwardFacing)
                    {
                        Create(floorIndex, floorProduct, hit);
                    }
                }
                else
                {
    ...
    private void Create(int index, ProductionDesign production, TrackableHit hit)
    {
        GameObject obj = production.AddItem(index, hit.Pose.position, hit.Pose.rotation);
